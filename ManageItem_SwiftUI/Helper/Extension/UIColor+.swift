...
extension UIColor {
    ...
    static let defaultColor = UIColor(hexString: "00BFFE")
    ...
    static var customBackground: Color {
        Color(UIColor { traitCollection in
            ...
            ? .black // 다크 모드: 완전 검정
            : .secondarySystemBackground // 라이트 모드: 약간 회색
        })
    }
    ...
    static var cardShadow: Color {
        Color(UIColor { trait in
            trait.userInterfaceStyle == .dark ? UIColor.white.withAlphaComponent(0.1) : UIColor.black.withAlphaComponent(0.15)
        })
    }
...
extension Color {
    ...
    static let customColor = Color(UIColor.customBackground)
    
    
    static let cardBackground = Color(UIColor.cardBackground)
